...
if __name__ == "__main__":
    ...
    # Training arguments
    parser.add_argument('--home', default="C:/Users/K1804053/OneDrive - King's College London/PycharmProjects")
...
datasets = {'mnist_dvs': r'mnist_dvs_events_new.hdf5',
            'dvs_gesture': r'dvs_gestures_events_new.hdf5'
            }


### Loading dataset & parameters
if args.dataset[:5] == 'mnist':
    args.dataset_path = args.home + r'/datasets/mnist-dvs/' + datasets[args.dataset]
elif args.dataset[:11] == 'dvs_gesture':
    args.dataset_path = args.home + r'/datasets/DvsGesture/' + datasets[args.dataset]
else:
    print('Error: dataset not found')

dataset = tables.open_file(args.dataset_path)
args.polarity = str2bool(args.polarity)
args.n_classes = dataset.root.stats.test_label[1]
args.x_max = dataset.root.stats.train_data[1]
...
### Network parameters
if args.polarity:
    args.n_input_neurons = int(2 * (args.x_max ** 2))
else:
    args.n_input_neurons = int(args.x_max ** 2)
args.n_output_neurons = 2  # Just labels [1, 7] for now
args.n_hidden_neurons = args.n_h


# Load custom topology for the network
...
args.rand_snr = str2bool(args.rand_snr)  # If SNR is randomized during training
dataset.close()

# Training
if args.model == 'neurojscc':
    train_neurojscc(args)  # Launch experiment with neurojscc

elif args.model == 'vqvae':
    train_vqvae_ldpc(args)  # Launch experiment with vqvae
