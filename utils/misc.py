...
def example_to_framed(example, args, T):
    if args.residual:
        frames = torch.zeros([args.n_frames, T // (args.n_frames - 1), args.x_max, args.x_max])
        frames[:-1] = example[:, :(args.n_frames - 1) * (T // (args.n_frames - 1))].transpose(1, 0).unsqueeze(0).unsqueeze(3).reshape(frames[:-1].shape)
        frames[-1, :args.residual] = example[:, -args.residual:].transpose(1, 0).unsqueeze(0).unsqueeze(3).reshape([args.residual, args.x_max, args.x_max])
    else:
        frames = torch.FloatTensor(example).transpose(1, 0).unsqueeze(0).unsqueeze(3).reshape([args.n_frames, T // args.n_frames,
                                                                                               args.x_max,
                                                                                               args.x_max])
...
def framed_to_example(frames, args, T):
    if args.residual:
        ...
        data_reconstructed = frames.reshape([-1, args.x_max ** 2]).transpose(1, 0)

    return data_reconstructed


def get_intermediate_dims(vqvae, args, T, dl):
    train_iterator = iter(dl)
    example, _ = next(train_iterator)
